...
const [CmddevinfoDrawer, drawerApi] = useVbenDrawer({
...
async function handleDelete(row: Required<CmddevinfoForm>) {
  await cmddevinfoRemove(row.id);
  await tableApi.query();
}

function handleMultiDelete() {
  const rows = tableApi.grid.getCheckboxRecords();
  const ids = rows.map((row: Required<CmddevinfoForm>) => row.id);
  Modal.confirm({
    title: '提示',
    okType: 'danger',
    content: `确认删除选中的${ids.length}条记录吗？`,
    onOk: async () => {
      await cmddevinfoRemove(ids);
      await tableApi.query();
    },
  });
}
...
function handleDownloadExcel() {
  commonDownloadExcel(cmddevinfoExport, '指令信息数据', tableApi.formApi.form.values, {
...
<template>
  <Page :auto-content-height="true">
    <BasicTable table-title="指令信息列表">
      <template #toolbar-tools>
        <Space>
          <a-button
            v-access:code="['iot:cmddevinfo:export']"
          ...
          <a-button
            type="primary"
            v-access:code="['iot:cmddevinfo:add']"
            @click="handleAdd"
          >
            {{ $t('pages.common.add') }}
          </a-button>
